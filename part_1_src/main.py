...
def create_augmented_dataloader(args, dataset):
    ...
    # Step 1: randomly select 5000 examples from the training set to transform
    aug_source = dataset["train"].shuffle(seed=42).select(range(5000))

    # Step 2: apply custom_transform to get the transformed 5000 examples
    transformed_aug = aug_source.map(custom_transform, load_from_cache_file=False)

    # Step 3: tokenize the transformed examples
    transformed_aug = transformed_aug.map(tokenize_function, batched=True, load_from_cache_file=False)
    transformed_aug = transformed_aug.remove_columns(["text"])
    transformed_aug = transformed_aug.rename_column("label", "labels")
    transformed_aug.set_format("torch")

    # Step 4: concatenate original tokenized training set with the 5000 transformed examples
    augmented_dataset = datasets.concatenate_datasets([tokenized_dataset["train"], transformed_aug])

    # Step 5: create and return the dataloader
